...
// Service class for open positions operations
export class OpenPositionsService {
  private formatPosition(position: PositionResult): string {
    try {
      ...
      const liquidationPrice = parseFloat(position.liquidation_price).toFixed(2);
      
      return `Market: ${position.market} | ${position.side} ${absSize} @ ${entryPrice} | PnL: ${unrealizedPnl} USD | Liq. Price: ${liquidationPrice}`;
    } catch (error) {
      console.error("Error formatting position:", error, "Position data:", position);
  ...
  async fetchOpenPositions(
    ...
    try {
      ...
      throw error instanceof ParadexOpenPositionsError
        ? error
        : new ParadexOpenPositionsError("Failed to fetch open positions", error);
    }
  }
  ...
  formatPositionsResponse(positions: PositionResult[]): {
    ...
    const formattedPositions = positions.map((position) => this.formatPosition(position));
    return {
...
export const paradexGetOpenPositions = async (
  ...
  try {
    ...
    // Format the response
    const formattedResponse = service.formatPositionsResponse(positionsData.results);
